...
$status_arr = [
    ...
    'direct_sale' => ['label' => "Direct Sale", 'color' => '#ff6b6b', 'text' => '#fff'] // NEW: Direct Sale status
];
...
// NEW: Total direct sales without any filter
...
// Total payments without any filter (INCLUDING DISCOUNT)
...
// CALCULATE ACTUAL CURRENT OUTSTANDING (Total Balance) - UPDATED WITH DIRECT SALES
$current_outstanding = $opening_bal + $total_repairs + $total_direct_sales - $total_payments;
...
// 3. Separate repair statuses, direct sales and payment filter
$repair_statuses = [];
$show_payments = false;
...
// If no status filter selected, show all (including payments and direct sales)
...
// If only payment is selected, set repair_statuses to empty array
// but ensure repairs are NOT shown
if(count($status_filters) == 1 && in_array('payment', $status_filters)) {
    $repair_statuses = []; // Empty array - no repairs should be shown
    $show_payments = true;
    $show_direct_sales = false;
}
...
if($repair_statuses !== null) {
    if(empty($repair_statuses)) {
        $pre_repair_sql .= " AND 1=0"; // No repairs if only payment or direct sale selected
    } else {
...
// NEW: Previous direct sales before date range (for filtered display)
...
// Brought forward balance for filtered display = Opening Balance + Previous Repairs + Previous Direct Sales - Previous Payments
$brought_forward = $opening_bal + $pre_repairs + $pre_direct_sales - $pre_payments_filter;

// 4. Filter Logic for current table (for display only)
$repair_filter = ""; 
$payment_filter = "";
$direct_sales_filter = ""; // NEW: Direct sales filter

if($from && $to){
    $repair_filter = " AND DATE(date_created) BETWEEN '{$from}' AND '{$to}' ";
    $payment_filter = " AND DATE(payment_date) BETWEEN '{$from}' AND '{$to}' ";
    $direct_sales_filter = " AND DATE(date_created) BETWEEN '{$from}' AND '{$to}' "; // NEW
} else if($from) {
...
if($repair_statuses !== null) {
    // If repair_statuses is empty array (meaning only payment or direct sale selected), don't show repairs
    if(empty($repair_statuses)) {
        $repair_filter .= " AND 1=0"; // This will ensure no repairs are fetched
    } else {
        $repair_filter .= " AND status IN (" . implode(',', $repair_statuses) . ")";
    }
}

// 5. Data Merging with DISCOUNT support (for filtered display) - UPDATED WITH DIRECT SALES
$ledger = [];
...
$ledger[] = [
    ...
    'discount' => 0,
    'timestamp' => 0,
    'date_completed' => null  // Added for consistency
];

// Repairs (Debit entries) - with or without status filter
// CHANGED: Added date_completed to SELECT query
...
while($row = $rep_res->fetch_assoc()){
    $ledger[] = [
        ...
        'date_completed' => $row['date_completed']  // Added date_completed
    ];
}

// NEW: Direct Sales (Debit entries) - only if show_direct_sales is true
...
// Payments (Credit entries with DISCOUNT) - only if show_payments is true
if($show_payments) {
...
// Get counts for each status - UPDATED WITH DIRECT SALES
$status_counts = [];
...
// Get period summaries for display - UPDATED WITH DIRECT SALES
...
<body>
    ...
    <div class="statement-container">
        ...
        <!-- COMPACT CLIENT INFO & BALANCE WITH PHOTO -->
        <div class="row mb-3 summary-row align-items-center">
            <div class="col-md-2 col-4 client-photo-container">
                <!-- Client Photo -->
                <img src="<?= validate_image($client['image_path']) ?>" 
                     alt="Client Image" 
                     class="client-photo" 
                     onerror="this.src='../../dist/img/no-image-available.png'">
            </div>
            <div class="col-md-4 col-8 client-info-mobile">
                ...
                <p class="mb-0" style="font-size: 10px; color: #6c757d;">Client ID: <?= $id ?></p>
            </div>
        ...
        <!-- DISPLAY PERIOD SUMMARY - UPDATED WITH DIRECT SALES -->
        <?php if($from || $to || !empty($status_filters)): ?>
        <div class="row mb-3 summary-row">
            <div class="col-12">
                    ...
                    <tr class="<?= $row['debit'] > 0 ? 'bg-dr' : ($row['credit'] > 0 ? 'bg-cr' : '') ?>">
                        ...
                        <td class="text-center">
                            ...
                            <?php elseif($row['status'] !== null): ?>
                                ...
                                <?php 
                                // CHANGED: Show delivered date for status 5 (Delivered)
            ...
            <div class="col-12">
                <div class="alert alert-info p-2" style="padding: 8px !important;">
                    ...
                    <div class="row">
                        ...
                        <div class="col-4">
                            <p class="mb-0" style="font-size: 10px;"><b>Net Balance:</b> ₹ <?= number_format($opening_bal + $total_repairs + $total_direct_sales - $total_payments, 2) ?></p>

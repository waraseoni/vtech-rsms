...
if (isset($_POST)) {
    ...
    if (empty($payment_id)) {
        ...
        $stmt = $conn->prepare("INSERT INTO client_payments 
                                (client_id, job_id, bill_no, amount, discount, payment_mode, payment_type, remarks) 
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        $stmt->bind_param("isddssss", $client_id, $job_id, $bill_no, $amount, $discount, $mode, $type, $remarks);
        ...
        $stmt->bind_param("ssddsssi", $job_id, $bill_no, $amount, $discount, $mode, $type, $remarks, $payment_id);
    }

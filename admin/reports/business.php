<?php 
require_once dirname(__DIR__) . '/inc/finance_calculations.php';

// Date Filter Logic
$from = isset($_GET['from']) ? $_GET['from'] : date("Y-m-01");
$to = isset($_GET['to']) ? $_GET['to'] : date("Y-m-t");

// Get finance data from centralized function
$finance = get_finance_data($from, $to, $conn);

// Dynamic Colors Logic
$profit_color = ($finance['net_profit'] >= 0) ? 'success' : 'danger';
$profit_text = ($finance['net_profit'] >= 0) ? 'Shuddh Laabh (Net Profit)' : 'Shuddh Haani (Net Loss)';

// Stock Value (separate - not in finance calculations)
            ...
            <div class="col-md-3">
                <div class="card shadow-sm border-0 border-left-lg border-info h-100">
                    <div class="card-body">
                        ...
                        <div class="h5 mb-0 font-weight-bold text-gray-800">₹ <?= number_format($finance['total_sales'], 2) ?></div>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card shadow-sm border-0 border-left-lg border-primary h-100">
                    <div class="card-body">
                        ...
                        <div class="h5 mb-0 font-weight-bold text-gray-800">₹ <?= number_format($finance['gross_profit'], 2) ?></div>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card shadow-sm border-0 border-left-lg border-<?= $profit_color ?> h-100 bg-<?= $profit_color ?>-light">
                    <div class="card-body">
                        ...
                        <div class="h5 mb-0 font-weight-bold text-<?= $profit_color ?>">₹ <?= number_format($finance['net_profit'], 2) ?></div>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
...
$grand_total_indirect_expense = $finance['total_outflow'];
$status_color = ($finance['net_profit'] >= 0) ? 'success' : 'danger';
?>
    ...
    <div class="row">
        <div class="col-md-8">
                ...
                <div class="card-body">
                    <table class="table table-bordered">
                        <tr>
                            ...
                            <td class="text-right">₹ <?= number_format($finance['total_sales'], 2) ?></td>
                        </tr>
                        <tr class="text-danger small">
                            <td>Estimated Parts Purchase Cost (90% of ₹<?= number_format($finance['total_parts_sold']) ?>)</td>
                            <td class="text-right">- ₹ <?= number_format($finance['parts_cost'], 2) ?></td>
                        </tr>
                        <tr class="bg-light font-weight-bold">
                            <td>Gross Operating Profit</td>
                            <td class="text-right text-primary">₹ <?= number_format($finance['gross_profit'], 2) ?></td>
                        </tr>
                    </table>
                </div>
            </div>
            ...
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body">
                    <table class="table table-bordered">
                        <tr><td>Staff Salary (Attendance Based)</td><td class="text-right">- ₹ <?= number_format($finance['salary'], 2) ?></td></tr>
                        <tr><td>Shop Expenses (Bills/Rent/Misc)</td><td class="text-right">- ₹ <?= number_format($finance['expenses'], 2) ?></td></tr>
                        <tr><td>Loan EMI Payments</td><td class="text-right">- ₹ <?= number_format($finance['loan_paid'], 2) ?></td></tr>
                        <tr><td>Discounts Given to Clients</td><td class="text-right">- ₹ <?= number_format($finance['repair_discount'], 2) ?></td></tr>
                        <tr class="bg-light font-weight-bold">
                            <td>Total Indirect Expenses</td>
                            <td class="text-right text-danger">₹ <?= number_format($finance['total_outflow'], 2) ?></td>
                        </tr>
                    </table>
                </div>
            </div>
            ...
            <div class="card shadow-sm border-<?= $status_color ?>">
                ...
                <div class="card-body">
                    <div class="text-center py-3">
                        ...
                        <h2 class="font-weight-bold text-<?= $status_color ?>">₹ <?= number_format($finance['net_profit'], 2) ?></h2>
                    </div>
                    <hr>
                    <ul class="list-group list-group-unbordered">
                        <li class="list-group-item"><b>Gross Profit:</b> <span class="float-right text-primary">₹<?= number_format($finance['gross_profit']) ?></span></li>
                        <li class="list-group-item"><b>All Expenses:</b> <span class="float-right text-danger">₹<?= number_format($finance['total_outflow']) ?></span></li>
                    </ul>
                    <div class="alert alert-<?= $status_color ?> mt-3 small">
                        <?php if($finance['net_profit'] >= 0): ?>
...
<?php 
// =========================================================
// ADVANCED BALANCE SHEET CALCULATIONS (Assets vs Liabilities)
// =========================================================
...
$cash_asset = ($finance['net_profit'] > 0) ? $finance['net_profit'] : 0;
...
$outstanding_salary = $finance['salary'];
$pending_shop_exp = $finance['expenses'];

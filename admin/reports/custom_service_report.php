...
<?php 
$from = isset($_GET['from']) ? $_GET['from'] : date("Y-m-01");  // Current month start
$to = isset($_GET['to']) ? $_GET['to'] : date("Y-m-t");          // Current month end
?>

<div class="card card-outline rounded-0 card-navy">
	<div class="card-header">
		<h3 class="card-title">Custom Service Report (Date Range)</h3>
	</div>
	<div class="card-body">
        ...
        <div class="container-fluid" id="printout">
            <div class="mb-3 text-center">
                <h4>Service Report (Labor Charges)</h4>
				...
				<tbody>
					...
					?>
						<tr>
							...
							<td><small class="text-muted"><?= htmlspecialchars($row['service_desc'] ?? '-') ?></small></td>
							<td class='text-right'><?= format_num($row['price']) ?></td>
						</tr>
					<?php endwhile; ?>
				</tbody>
                <tfoot>
                    <th class="py-1 text-center" colspan="5">Total Service Charges (<?= date("d/m/Y", strtotime($from)) ?> to <?= date("d/m/Y", strtotime($to)) ?>)</th>
    ...
    <div class="d-flex w-100 mb-3">
        ...
        <div class="col-8 text-center">
            <div style="line-height:1em">
                ...
                <h3 class="text-center mb-0"><b>Service Report (Labor Charges)</b></h3>
...
<script>
	$(document).ready(function(){
		$('#filter-form').submit(function(e){
            e.preventDefault();
            location.href = "./?page=reports/custom_service_report&" + $(this).serialize();
        });
        ...
        $('#print').click(function(){
            ...
            h.find('title').text('Service Report - <?= date("d-m-Y", strtotime($from)) ?> to <?= date("d-m-Y", strtotime($to)) ?>');

...
while($row = $qry->fetch_assoc()) {
    ...
    $job_display = '<div class="d-flex flex-column" style="line-height:1.3;"><small class="text-primary font-weight-bold"><i class="fa fa-tag mr-1"></i><a href="./?page=transactions/view_details&id='.$row['id'].'">' . htmlspecialchars($row['job_id']) . '</a></small><small class="text-danger"><i class="fa fa-barcode mr-1"></i><a href="./?page=transactions/view_details&id='.$row['id'].'" class="text-danger">' . htmlspecialchars(!empty($row['code']) ? $row['code'] : 'No Code') . '</a></small></div>';
    
    $client_display = '<div class="client-cell">' . $avatar_html . '<div class="d-flex flex-column" style="line-height:1.3;"><span class="font-weight-bold"><a href="./?page=clients/view_client&id='.$row['client_name'].'" class="text-dark">' . htmlspecialchars($fullname) . '</a></span>' . $bal_display . '<small class="text-success">' . ($contact ? '<a href="https://wa.me/91'.preg_replace('/\D/', '', $contact).'" target="_blank" class="text-success"><i class="fab fa-whatsapp mr-1"></i>' . htmlspecialchars($contact) . '</a>' : '') . '</small></div></div>';
    ...
    $wa_params = "'" . addslashes($row['job_id']) . "', '" . addslashes($contact) . "', '" . $row['amount'] . "', '" . addslashes($fullname) . "', '" . addslashes(!empty($row['code']) ? $row['code'] : 'No Code') . "', '" . addslashes($row['item']) . "', '" . $status . "'";
    $action_display = '<button type="button" class="btn btn-flat btn-default btn-sm dropdown-toggle dropdown-icon" data-toggle="dropdown">Action</button><div class="dropdown-menu" role="menu"><a class="dropdown-item" href="./?page=transactions/view_details&id=' . $row['id'] . '"><span class="fa fa-eye text-primary"></span> View</a><div class="dropdown-divider"></div><a class="dropdown-item" href="javascript:void(0)" onclick="sendWA(' . $wa_params . ')"><span class="fab fa-whatsapp text-success"></span> WhatsApp</a><div class="dropdown-divider"></div><a class="dropdown-item" href="../pdf/bill_template.php?job_id=' . htmlspecialchars($row['job_id']) . '" target="_blank"><span class="fa fa-print text-info"></span> Print Bill</a><div class="dropdown-divider"></div><a class="dropdown-item combined-invoice-btn" href="javascript:void(0)" data-client="' . $row['client_name'] . '" data-txid="' . $row['id'] . '"><span class="fa fa-file-invoice text-warning"></span> Combined Invoice</a><div class="dropdown-divider"></div><a class="dropdown-item edit_data" href="./?page=transactions/manage_transaction&id=' . $row['id'] . '"><span class="fa fa-edit text-info"></span> Edit</a><div class="dropdown-divider"></div><a class="dropdown-item edit_data" href="./?page=transactions/manage_transaction_old&id=' . $row['id'] . '"><span class="fa fa-edit text-info"></span> Old Edit</a><div class="dropdown-divider"></div><a class="dropdown-item delete_data" href="javascript:void(0)" data-id="' . $row['id'] . '"><span class="fa fa-trash text-danger"></span> Delete</a></div>';

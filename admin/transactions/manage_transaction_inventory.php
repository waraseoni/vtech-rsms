                    ...
                    <div class="row g-2 mb-3">
                        <div class="col-md-6">
                            ...
                            <select name="client_name" id="client_name" class="form-control form-control-sm border-secondary rounded-0 bg-white select2" required>
                                <option value="" disabled <?= !isset($client_name) ? "selected" : "" ?>>Select Client</option>
                                <?php 
                                ...
                                ?>
                                <option value="<?= $row['id'] ?>" <?= isset($client_name) && $client_name == $row['id'] ? 'selected' : '' ?>>
                                    <?= $row['name'] ?>
                                </option>
                                <?php endwhile; ?>
                            </select>
                                ...
                                <div class="row g-2 mb-2">
                                    <div class="col-9">
                                        <select id="product_sel" class="form-control form-control-sm border-secondary rounded-0 bg-white">
                                            ...
                                            <?php 
                                            // Modified query: only products with quantity > 0 in inventory
                                            $products = $conn->query("
                                                SELECT p.*, i.quantity 
                                                FROM product_list p 
                                                INNER JOIN inventory_list i ON p.id = i.product_id 
                                                WHERE p.delete_flag = 0 AND p.status = 1 AND i.quantity > 0 
                                                ORDER BY p.name
                                            ");
                                            ...
                                            <option value="<?= $row['id'] ?>" 
                                                ...
                                                <?= $row['name'] ?> (₹<?= number_format($row['price'],2) ?>) - Stock: <?= $row['quantity'] ?>
                                            </option>
                                            <?php endwhile; ?>
                                        </select>
                                    </div>
...
$(function(){
    ...
    $('#add_product').click(function(){
        ...
        let stock = $('#product_sel option:selected').data('stock'); // available stock
        ...
        tr.find('.product_qty_input').attr('max', stock).val(1); // set max to stock, default 1
    ...
    // Validate product quantity against stock (when user changes quantity)
    ...
    $(document).off('submit', '#transaction-form').on('submit', '#transaction-form', function(e) {
        ...
        // Optional: double-check product quantities against stock before submit
